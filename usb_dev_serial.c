                        ...
                        uint32_t ui32MsgValue, void *pvMsgData)
{
    ...
    switch(ui32Event)
    {
        ...
        case USBD_CDC_EVENT_GET_LINE_CODING:
        	...
        	memcpy(pvMsgData,&ConfigLinea,sizeof(ConfigLinea));//Como es un puerto emulado, respondo con la configuraciï¿½n memorizada.
            break;
...
//          void *pvMsgData)
{
    ...
    switch(ui32Event)
    {
        case USB_EVENT_TX_COMPLETE:
            ...
            // here. etsit:ï¿½cambiar para adaptar? --> ï¿½Leer datos de la fifo?
            //
...
int32_t xUSBSerialWrite( uint8_t *buffer, int32_t size, portTickType xBlockTime )
{
	int32_t i;
	uint32_t ulSpace;
	//AJR - Protegemos la función con un Mutex para que varias tareas puedan acceder a ella sin provocar errores
	//AJR - Coge el Mutex
	...
	if (ulSpace>=size)
	{
		...
		//AJR - Tras escribir, suelta el Mutex
